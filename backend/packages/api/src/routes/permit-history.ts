...
import { getPermitHistoryResultRepository } from '@bones-report/shared';
...
/**
 * GET /permit-history
 * List permit histories, optionally filtered by address request.
 */
  ...
  try {
    ...
    if (!result) {
      return res.status(404).json({
        error: 'Not Found',
        message: `PermitHistoryResult with id '${req.params.id}' not found`,
        timestamp: new Date().toISOString(),
      });
    }
...
export { router as permitHistoryRouter };
